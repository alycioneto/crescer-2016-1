'use strict';
 ...
 /*$(document).ready(function () {
  executa tudo que estiver aqui, depois que souber o que a página possui.
 }); */
...
$('button[name=btnCarregaCavaleiro]').click(function() {
    $('#imagens').empty();
  goldSaints.forEach(function (e) {
      var $imagem = $('<img>').attr('src', e.imagens[0].url).attr('alt', e.nome).attr('id', e.id);
      $('#imagens').append($('<li>').append($imagem));
    });
  ...
  $('#imagens img').mouseover(function() {
    ...
    for(var campo in cavaleiro) {
      $('#infoCavaleiros').append($('<strong>').text(campo+ ': ')).append(cavaleiro[campo]).append($('<br>'));
    }

  });

  $('#imagens img').mouseout(function() {
    $('#infoCavaleiros').empty();
  });
