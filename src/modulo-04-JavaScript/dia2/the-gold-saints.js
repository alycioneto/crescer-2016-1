...
function obterDoadores(){
  ...
  for( var i = 0 ; i < goldSaints.length; i++){
      goldSaints[i].tipoSanguineo === 'O'? doadores.push(goldSaints[i]) : 0 ;
  }
  return doadores;
};
...
function obterMesesComMaisAniversarios (){
  ...
  var mesesCavaleiros =[];
  goldSaints.reduce(function(total, proximo){ return mesesCavaleiros.push(new Date(proximo.dataNascimento))});
  mesesCavaleiros.sort(function(a,b){return a.getMonth()- b.getMonth()});
  var max=0,result,freq = 0;
    for(var i=0; i < mesesCavaleiros.length-1; i++){
      mesesCavaleiros[i].getMonth()===mesesCavaleiros[i+1].getMonth() ? freq++: freq=0;
        if(freq>max){
            result = mesesCavaleiros[i];
            max = freq;
        }
    }
};
...
function obterAlturaMedia(){
  ...
  for (var i in goldSaints) {
    ...
    var mediaEmMetro = (soma/12)/100;
    return Math.round(mediaEmMetro * 100) / 100;

};

function obterAlturaMediana(){
  var arrayOrdenado = goldSaints.sort(function(a,b){return a.alturaCm - b.alturaCm});
  var meioDoArray = arrayOrdenado.length/2;
  var mediana;
  if (arrayOrdenado.length%2 === 0){
    medianaEmMetro =(goldSaints[meioDoArray-1].alturaCm + goldSaints[meioDoArray].alturaCm) /2;
     return Math.round(medianaEmMetro * 100) / 100;
  }
  else {
    mediana =(goldSaints[meioDoArray.toPrecision(1)].alturaCm)/100;
     return Math.round(medianaEmMetro * 100) / 100;
  }
}

function obterPesoMedio(cavaleiros){
   var calculaPesoMedio = 0, contCav = 0;
   for (var i = 0; i < cavaleiros.length; i++){
     if(cavaleiros[i].pesoLb !== undefined){
       calculaPesoMedio += cavaleiros[i].pesoLb;
       contCav++;
     }
   }

   calculaPesoMedio = (calculaPesoMedio / contCav) * 0.45;
   return parseFloat(calculaPesoMedio.toFixed(2));
}

function obterPesoMedioDoadores(){
 var arrayDoadores = obterDoadores();
    return obterPesoMedio(arrayDoadores);
}
