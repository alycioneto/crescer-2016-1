...
$(function () {
    ...
    setInterval(function () {
        $.get('/Cavaleiro/Get').done(function (e) {
            tamanhoLista = $("#cavaleiros li").length;
            var quantidadeCavaleiros = e.data.length;
            if (tamanhoLista < quantidadeCavaleiros) {
                for (var i = tamanhoLista; i < quantidadeCavaleiros; i++) {
                    $('#cavaleiros').append(new CavaleiroIndexView().criarHtmlCavaleiro(e.data[i]));
                }
                notificacaoNovosCavaleiros(quantidadeCavaleiros - tamanhoLista);
            }
        })
    }, 5000);
});
function notificacaoNovosCavaleiros(quantidadeNovosCavaleiros) {
    Notification.requestPermission().then(function (result) {
        if (result === 'granted') {
            var options = {
                body: quantidadeNovosCavaleiros + ' novos cavaleiros foram adicionados!',
                icon: 'http://doyer.guyllaume.free.fr/creations/vectorisations/tn_Pegasus_box_by_NinjaShaka.jpg'
            }
